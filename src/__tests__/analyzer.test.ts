...
const defaultOptions = {
  ...
  cache: false,
  cacheDir: "",
  limits: { maxTokens: 4000, timeoutMs: 60_000 },
};
...
describe("analyzeRoutes", () => {
  ...
  it("handles a content filter 400 error gracefully", async () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const filterError = Object.assign(
    ...
    expect(results[0].pathItem).toEqual({});
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining("Content filter"),
    );
    warn.mockRestore();
  });
  ...
  it("re-throws non-content-filter errors", async () => {
    ...
    await expect(analyzeRoutes([makeRoute()], defaultOptions)).rejects.toThrow(
      "Network timeout",
    );
  });
  ...
  it("processes multiple routes independently", async () => {
    const routeA = makeRoute({
      urlPath: "/api/users",
      sourceCode: "GET users",
    });
    const routeB = makeRoute({
      urlPath: "/api/posts",
      sourceCode: "GET posts",
    });

    vi.mocked(generateText)
      .mockResolvedValueOnce({
        text: '{"get":{"operationId":"listUsers"}}',
      } as Awaited<ReturnType<typeof generateText>>)
      .mockResolvedValueOnce({
        text: '{"get":{"operationId":"listPosts"}}',
      } as Awaited<ReturnType<typeof generateText>>);
  ...
  it("uses getModelId for the cache key", async () => {
    ...
    vi.mocked(generateText).mockResolvedValue({ text: "{}" } as Awaited<
      ReturnType<typeof generateText>
    >);

    await analyzeRoutes([makeRoute()], {
      ...defaultOptions,
      provider: "anthropic",
    });
