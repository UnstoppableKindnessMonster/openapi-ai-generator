...
function makeRoute(urlPath: string, pathItem: Record<string, unknown> = {}): AnalyzedRoute {
...
describe('assembleSpec', () => {
	...
	it('includes description when provided', () => {
		const spec = assembleSpec(
			makeConfig({ openapi: { title: 'T', version: '1', description: 'My API', servers: [] } }),
			[],
		);
	...
	it('includes servers when provided', () => {
		...
		const spec = assembleSpec(
			makeConfig({ openapi: { title: 'T', version: '1', description: '', servers } }),
			[],
		);
	...
	it('maps analyzed routes to paths', () => {
		...
		expect(spec.paths['/api/users']).toEqual({ get: { operationId: 'listUsers' } });
		expect(spec.paths['/api/posts']).toEqual({ get: { operationId: 'listPosts' } });
	});
